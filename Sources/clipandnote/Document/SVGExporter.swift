...
enum SVGExporter {
    ...
    private static func text(_ o: MarkupObject) -> String {
        ...
        // paint-order:stroke draws the contrast outline behind the fill, like the
        // canvas's negative-strokeWidth outlined text.
        return "<text x=\"\(x)\" y=\"\(num(o.frame.minY + o.fontSize * 0.82))\" "
            + "font-family=\"\(escape(family))\" font-size=\"\(num(o.fontSize))\" font-weight=\"600\" "
            + "fill=\"\(color)\" stroke=\"\(contrast)\" stroke-width=\"3\" paint-order=\"stroke\">\(tspans)</text>\n"
    }
